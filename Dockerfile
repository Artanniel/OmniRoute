...
RUN --mount=type=cache,id=s/c34594b1-a82f-4808-a8da-fe0f2a3331f0-/var/cache/apt,target=/var/cache/apt,sharing=shared \
  --mount=type=cache,id=s/c34594b1-a82f-4808-a8da-fe0f2a3331f0-/var/lib/apt/lists,target=/var/lib/apt/lists,sharing=shared \
  apt-get update \
  && apt-get upgrade -y \
...
RUN --mount=type=cache,id=s/c34594b1-a82f-4808-a8da-fe0f2a3331f0-/root/.npm,target=/root/.npm \
  npm ci --no-audit --no-fund --legacy-peer-deps --ignore-scripts \
  && npm rebuild better-sqlite3 --foreground-scripts --ignore-scripts=false \
...
COPY . ./
RUN --mount=type=cache,id=s/c34594b1-a82f-4808-a8da-fe0f2a3331f0-/app/.build/next/cache,target=/app/.build/next/cache \
...
# `npm run build` (build-next-isolated → assembleStandalone) bundles ALL runtime
# files into .build/next/standalone/ — .next, node_modules, migrations, scripts,
# docs, and the extra modules covered by scripts/build/assembleStandalone.mjs
...
# (migrations, scripts/, src/mitm/server.cjs) that this COPY doesn't touch.
#
# rm -rf first: npm workspaces (workspaces: ["open-sse"]) materialize
# node_modules/@omniroute/open-sse as a symlink in the builder, but the
# standalone COPY above already placed a real directory at that same path.
# BuildKit's COPY refuses to replace a directory with a file/symlink in an
# overlay merge ("cannot replace to directory ... with file"), so clear the
# destination first and let this COPY fully replace it instead of merging.
...
RUN --mount=type=cache,id=s/c34594b1-a82f-4808-a8da-fe0f2a3331f0-/var/cache/apt,target=/var/cache/apt,sharing=locked \
  --mount=type=cache,id=s/c34594b1-a82f-4808-a8da-fe0f2a3331f0-/var/lib/apt/lists,target=/var/lib/apt/lists,sharing=locked \
  apt-get update \
